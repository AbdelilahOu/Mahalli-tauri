...
pub fn get_all_sellers(connection: &mut SqliteConnection) -> Vec<Value> {
    ...
    response.into_iter().for_each(|(fname, s_id)| {
        result.push(json!({
            "label": fname,
            "value": format!("{}",s_id)
        }))
...
pub fn get_seller(s_id: String, connection: &mut SqliteConnection) -> Seller {
    let result = sellers::dsl::sellers
        .find(&s_id)
...
pub fn delete_seller(s_id: String, connection: &mut SqliteConnection) -> usize {
